...
在很多编程语言中，线程池极大简化了创建线程来完成特定工作的操作。借助Beam虚拟机的特性，我们可以在一个进程中轻松创建成千上万个微进程，要比其他语言要节约更多的内存，而这个优势在Elixir中并不是很有用。相反，我们还需要工作池来限制我们进程的数量，从而保证与其交互的系统不会资源溢出。

Poolboy是个简洁可靠的库，它可以用来在Elixir应用中创建线程池。你只需要定义个池的名字，工作线程的内容和可用工作线程的数量，Poolboy就能运转。后续就是一个简单的利特尔法则的计算公式。

利特尔法则内容为：在一个静止系统中，平均的消费者数量`L`等于平均的请求到达率`λ`乘以每个消费者在系统中平均停留时间`W`。我们可以使用这个简单公式来算出所需的工作线程数量，用于消除Poolboy的瓶颈，你也会认识到这个公式影响在你应用中的几乎无处不在。
